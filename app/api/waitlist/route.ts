...
export async function POST(request: Request) {
  try {
    ...
    if (existingCustomer) {
      // Customer exists — ensure "waitlist" tag is present
      const currentTags: string[] = existingCustomer.tags || [];
      if (!currentTags.includes("waitlist")) {
        await adminGraphQL(
          `mutation AddWaitlistTag($input: CustomerInput!) {
    ...
    // 2. Create new customer with waitlist tag + marketing consent
    ...
    if (createResult?.errors?.length > 0) {
      console.error("[Waitlist] GraphQL errors:", createResult.errors);
      return NextResponse.json(
    ...
    if (userErrors?.length > 0) {
      console.error("[Waitlist] Customer creation errors:", userErrors);
