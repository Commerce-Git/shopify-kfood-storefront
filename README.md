...
> **작업 시작점:** [공통 플랫폼 기준](docs/platform-analysis/README.md)과 [목표·확정 결정](docs/platform-analysis/PROJECT_DIRECTION.md)을 먼저 확인하세요. 현재 사업은 한국 제조 상품 중심이며, 프론트·어드민을 하나의 플랫폼으로 분석합니다. 아래 과거 구현 설명은 해당 코드와 검증 기록을 대조해 사용하세요.

## 1. 프로젝트 개요

한국 제조 상품의 탐색·구매·주문 확인·고객 지원을 제공하는 Shopify 헤드리스 프론트 프로젝트입니다. 작가·상품·물류·정산 업무는 `blank-seoul-admin`과 연결됩니다. 세부 상품군·판매 시장·출시 방식은 공통 목표 문서의 확정/미결정 상태를 따릅니다.

- **아키텍처**: Headless Commerce (Next.js 프론트엔드 + Shopify 결제 백엔드)
- **목표**: 작가의 안정적인 판매·정산, 구매자의 신뢰, 감당 가능한 비용·인력으로 거래를 반복·확장
- **브랜드 이름**: Blank Seoul
- **법인명**: Blank Palette LLC (DBA: 추후 결정)

## 2. 관련 프로젝트 및 아키텍처

이 프로젝트는 고객 화면과 관련 서버 API를 담당합니다. `blank-seoul-admin`은 작가·운영·물류 포털을 담당합니다. Supabase는 공유 업무 데이터, Shopify는 카탈로그·체크아웃·결제/주문 연동을 맡으며, 데이터별 원본과 변경 책임은 [데이터·업무 흐름](docs/platform-analysis/DATA_AND_FLOWS.md)을 따릅니다.

```text
blank-seoul-storefront  ← 문의 등 API 연동 →  blank-seoul-admin
          ↓                                     ↓
  Supabase 공유 업무 데이터 / Shopify 카탈로그·결제·주문 연동
```

## 3. 기술 스택

아래 3절 이후는 과거 작성된 구현 메모입니다. API 버전·키 이름·일정·완료 주장을 최신 운영 상태로 간주하지 않습니다. 현재 조사 근거와 미검증 항목은 [플랫폼 분석](docs/platform-analysis/README.md)에 기록합니다.
...
## 4. 기존 핵심 구현 메모 — 현재 코드와 대조 필요

아래는 기존 구현 의도의 기록입니다. 현재 동작은 실제 코드와 해당 실행의 검증 결과를 기준으로 판단합니다.
